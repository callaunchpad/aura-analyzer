...
def get_args():
    ...
    parser.add_argument(
        "--model_dir",
        "-m",
        default="../../color-correction/PyTorch/models",
    ...
    parser.add_argument(
        "--output_dir", "-o", default="../result_images", help="Directory to save the output images", dest="out_dir"
    )
    parser.add_argument(
        "--task", "-t", default="all", help="Specify the required task: 'AWB', 'editing', or 'all'.", dest="task"
    )
    parser.add_argument(
        "--target_color_temp",
        "-tct",
...
if __name__ == "__main__":
    ...
    if args.task.lower() == "all":
        if (
            os.path.exists(os.path.join(args.model_dir, "net_awb.pth"))
            and os.path.exists(os.path.join(args.model_dir, "net_t.pth"))
            and os.path.exists(os.path.join(args.model_dir, "net_s.pth"))
        ):
            # load awb net
            ...
            net_awb.load_state_dict(torch.load(os.path.join(args.model_dir, "net_awb.pth"), map_location=device))
            net_awb.eval()
            # load tungsten net
            ...
            net_t.load_state_dict(torch.load(os.path.join(args.model_dir, "net_t.pth"), map_location=device))
            net_t.eval()
            # load shade net
            ...
            net_s.load_state_dict(torch.load(os.path.join(args.model_dir, "net_s.pth"), map_location=device))
            net_s.eval()
        ...
        if os.path.exists(os.path.join(args.model_dir, "net_t.pth")) and os.path.exists(
            os.path.join(args.model_dir, "net_s.pth")
        ):
            # load tungsten net
            net_t = deep_wb_single_task.deepWBnet()
            logging.info("Loading model {}".format(os.path.join(args.model_dir, "net_t.pth")))
...
if args.task.lower() == "all":  # awb and editing tasks
    out_awb, out_t, out_s = deep_wb(
        img, task=args.task.lower(), net_awb=net_awb, net_s=net_s, net_t=net_t, device=device, s=S
    )
    ...
    if tosave:
        result_awb = utls.to_image(out_awb)
        result_awb.save(os.path.join(out_dir, "temp.png"))
